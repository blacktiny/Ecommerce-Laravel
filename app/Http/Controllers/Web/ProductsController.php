...
class ProductsController extends DataController
{
	...
	public function productDetail(Request $request){
		...
		if (Auth::guard('customer')->check()) {
			...
			$categories = DB::table('customers_to_categories')
					->leftJoin('categories','categories.categories_id','=','customers_to_categories.categories_id')
					->leftJoin('categories_description','categories_description.categories_id', '=', 'categories.categories_id')				
					->where('customers_id','=', $customers_id)->where('categories_description.language_id','=', $language_id )
					->select('categories_name')
					->get();
			
			$arr_Category = [];
			foreach($categories as $category) {
				array_push($arr_Category, $category->categories_name);
			}
